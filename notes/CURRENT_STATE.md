...
## Dolphin 26.08 снова открывал домашнюю папку вместо последней — исправлено, 2026-08-24

После обновления с 26.04.3 до 26.08.0 пустой запуск Dolphin создавал новый
GUI-процесс, если одно окно уже было открыто. В 26.08 восстановление вкладок
выполняется только первым GUI-процессом; поэтому повторные клики по значку
открывали `/home/ediskrad`, а несколько процессов конкурировали за общий файл
сохранённой сессии.

Добавлен `tools/dolphin-launcher.sh`, устанавливаемый как
`~/.local/bin/tahoe-dolphin`. Пустой запуск (в том числе `Meta+E`) всегда
получает URL из последней сохранённой сессии вместо домашней папки — вне
зависимости от количества других окон. Проверка только по PID здесь
недостаточна: сразу после закрытия процесса его D-Bus-сервис может ещё кратко
существовать, из-за чего новый Dolphin пропускает штатное восстановление.
Явный URL устраняет эту гонку и позволяет открыть место из последнего
закрытого окна, пока другие окна Dolphin остаются запущенными.
Запуски с явно заданным URL передаются без изменения. Desktop override и
авторемонт проверяют и восстанавливают этот launcher.

Launcher читает `Active Tab Index` и URL соответствующего `Tab Data N`;
читать всегда `Tab Data 0` нельзя, потому что закрытое окно может сохранить
активной вторую или последующую вкладку.

Для `Meta+E` установлен отдельный desktop-компонент
`tahoe-dolphin-shortcut.desktop`. Штатный компонент
`org.kde.dolphin.desktop` сохранял в KGlobalAccel системную команду
`/usr/bin/dolphin` даже после изменения пользовательского `Exec`, поэтому
сочетание обходило launcher; сочетание у штатного override удалено.

## Автовосстановление патчей после обновлений — 2026-08-24

Установлен пользовательский systemd-таймер
`tahoe-theme-auto-repair.timer`. Через 5 минут после запуска и затем каждые
6 часов `tools/repair-theme-after-update.sh` проверяет и при необходимости
восстанавливает Overview backdrop blur, системный `forceblur.so`, KWin Window
Rules и Dolphin alpha-hook. Повторные запуски защищены `flock`.

Сборка Overview перенесена из переполненного `/tmp` в `tmp/` проекта,
компилятор также получает этот каталог через `TMPDIR`; число потоков по
умолчанию ограничено четырьмя. Патч повторно собран и установлен для KWin
6.7.4 build107. Для его загрузки нужен полный перезапуск KWin (выход из
сеанса или перезагрузка).

В Dolphin явно включено `RememberOpenedTabs=true`. Пользовательский desktop
override запускает Dolphin с `LANG=ru_RU.UTF-8`, `LANGUAGE=ru` и
`LC_ALL=ru_RU.UTF-8`, чтобы preload-запуск не переключал интерфейс на
английский язык.

## Диалоги Dolphin (Свойства и т.п.): иконки/текст были прозрачными — исправлено настоящей поканальной альфой — 2026-08-21

Пользователь заметил на скриншоте диалога «Свойства» (вкладка «Основное»):
шрифт в главном окне Dolphin отображается нормально, а в самом диалоге
иконка файла и текст выглядят нечёткими и полупрозрачными — хотя прозрачность
и блюр должны касаться только фона.

**Причина.** Главное окно Dolphin получает от Kvantum настоящую поканальную
альфу (красится только `base.color`/`window.color`, текст и иконки рисуются
поверх уже непрозрачными). Диалоги (`QDialog`, например `KPropertiesDialog`)
— нет: Kvantum даёт окну реальную альфу, только если успевает выставить
`Qt::WA_TranslucentBackground` ДО того, как у окна создастся нативная
поверхность (`style/polishing.cpp` в исходниках Kvantum,
`~/whitesur-build/Kvantum/Kvantum/`, строки ~374-391: если
`window->format().alphaBufferSize() != 8` на момент polish — Kvantum
навсегда отказывается от прозрачности для этого окна). У диалогов Dolphin/KIO
нативное окно реализуется раньше — при вызове `winId()` в конструкторе (нужно
для установки transient-for родителя), то есть до того, как Kvantum вообще
видит виджет. Подтверждено напрямую: отдельная тестовая Qt6-программа,
имитирующая `QDialog`, вызывающий `winId()` до `show()`, показывает
`alphaBufferSize()==0`; `QMenu` (не делает этого) — `==8`, что и объясняет,
почему выпадающие меню уже были прозрачны корректно, а диалоги — нет. Из-за
этого единственным работающим механизмом для диалогов оставалось компоузерное
правило KWin `opacityactive=75%` (тот же приём, что и для System
Settings/KCM) — а оно множит альфу ВСЕЙ уже отрисованной текстуры окна
целиком, включая иконки и текст, а не только фон.

**Фикс — `source/kvantum-dialog-alpha-fix/`.** `winid-alpha-hook.cpp`:
маленькая `LD_PRELOAD`-библиотека, перехватывающая экспортируемый символ
`QWidget::winId()` (`_ZNK7QWidget5winIdEv`) — если у `QDialog` ещё нет
`Qt::WA_WState_Created`, перед вызовом настоящего `winId()` выставляет
`Qt::WA_TranslucentBackground`/`Qt::WA_NoSystemBackground` (именно то, что
позже сделал бы сам Kvantum, только заранее). Проверено экспериментально на
том же тестовом бинарнике: с preload'ом diалог с ранним `winId()` тоже
получает `alphaBufferSize()==8`. Три компонента фикса вместе:

1. `tools/install-kvantum-dialog-alpha-fix.sh` — собирает
   `~/.local/lib/kvantum-dialog-alpha-fix.so` и устанавливает
   пользовательский override `~/.local/share/applications/org.kde.dolphin.desktop`
   (копия системного с `Exec=env LD_PRELOAD=... dolphin %u`).
2. `Kvantum/MacTahoe/MacTahoe.svg`, `dialog-normal`/`dialog-normal-inactive`
   — `opacity="1"` → `opacity="0.75"` (та же альфа, что у `window-normal`) —
   раньше поверхность окна получала прозрачность, но сама заливка была
   полностью непрозрачной (`fill-opacity:1`), так что без этой правки эффект
   был бы всё равно не виден.
3. `tools/install-kwin-window-rules.sh` — новая переменная
   `NATIVE_ALPHA_FULL_CLASSES` (по умолчанию `dolphin`): класс исключается из
   компоузерного правила ПОЛНОСТЬЮ (главное окно И диалоги), в отличие от
   старого `NATIVE_ALPHA_CLASSES` (теперь только `Alacritty`), который
   по-прежнему исключает лишь окна без `hastransientparent` (только главное
   окно). Без этого шага фон диалога получал бы двойное умножение альфы
   (0.75×0.75≈0.56 — тот же класс бага, что уже чинили для главного окна
   Dolphin/Alacritty, см. «Баг: двойное умножение alpha» ниже) — именно это
   и произошло в первой живой проверке: фон диалога был заметно темнее рамки,
   пока дополнительно не исключили диалоги из общего правила.

**Проверено строго, по пикселям**, тем же методом, что и везде в этом файле
— диалог «Свойства» перемещался по экрану поверх разных обоев, скриншоты
сравнивались попиксельно: после фикса цвет фона диалога зависит от обоев за
окном (реальная прозрачность+блюр), а цвет иконки/текста — нет (остаются
чёткими, не зависят от фона). До фикса было наоборот: и фон, и иконка
одинаково "плыли" вместе с обоями. Пользователь подтвердил визуально на живой
системе: после первого прохода фон диалога отличался от рамки окна (двойное
умножение альфы) — исправлено добавлением `dolphin` в
`NATIVE_ALPHA_FULL_CLASSES`; после этого иконки и текст больше не
прозрачны, а фон и рамка совпадают.

**Важно на будущее:** Dolphin — D-Bus singleton-приложение (`KDBusService`),
обычный повторный запуск `dolphin` не создаёт новый процесс, а просто
просит уже работающий отдать команду (LD_PRELOAD новой попытки запуска не
подействует). Чтобы подхватить `.so`/новый `.desktop`, нужно закрыть все
процессы (`pkill -9 -x dolphin`, не `-f "/usr/bin/dolphin"` — реальный
`argv[0]` может быть без пути) и запустить заново. Тот же приём в принципе
переносим на любое другое Kvantum/Qt-приложение с той же проблемой (KCalc,
System Settings и т.д.) — достаточно добавить его в
`NATIVE_ALPHA_FULL_CLASSES` и завести аналогичный override `.desktop` с тем
же `.so`, предварительно проверив тем же тестовым Qt6-бинарником, что его
диалоги действительно страдают от раннего `winId()` (не факт, что у всех
приложений причина та же).

## KWin Overview: исправлена неполная установка QML — 2026-08-06, 16:25 MSK

После перезагрузки Overview не открывался. Журнал KWin показал точную ошибку:
`Cannot assign to non-existent property "backdropSource"`. Пересобранный
`kwin_wayland` уже содержал патченный `Main.qml`, но сценарий устанавливал
только бинарник и не устанавливал изменённый системный
`WindowHeapDelegate.qml` из пакета `kwin-common`.

Патченный `WindowHeapDelegate.qml` установлен в
`/usr/lib/x86_64-linux-gnu/qt6/qml/org/kde/kwin/private/effects/`, а штатная
копия сохранена как
`/var/backups/tahoe-kwin/WindowHeapDelegate-6.7.4-stock.qml`. Сценарии сборки
и отката обновлены: теперь бинарник и QML-файл устанавливаются и
восстанавливаются вместе. Текущий процесс KWin кэширует старое описание
QML-типа, поэтому для активации исправления нужен ещё один выход из сеанса и
повторный вход либо перезагрузка.

## KWin Overview: исправленная сборка установлена — 2026-08-06, 16:19 MSK

После подтверждения, что рабочий стол со штатным KWin работает нормально,
Overview-патч собран повторно. Из сценария удалён
`-DKWIN_BUILD_SCREENLOCKER=OFF`; CMake нашёл `KScreenLocker`, а собранный
бинарник до установки успешно прошёл обе проверки:
`kwin_wayland --version` и `kwin_wayland --no-lockscreen --version`.

Установлен `/usr/bin/kwin_wayland` с SHA-256
`cd1895763e76b096cbf45ac16526961f09850431659721d3ffbae0a9ae022584`.
Штатная копия с SHA-256 `a0b4937d81e53824746cf4e43812fd3c943baf27fe7e33096224c8c5cd8b7cf8`
сохранена в `/var/backups/tahoe-kwin/kwin_wayland-6.7.4-stock`.

**Текущее состояние:** новая сборка установлена на диск, но запущенный процесс
KWin всё ещё использует удалённый старый inode штатного бинарника. Нужен выход
из сеанса и повторный вход либо перезагрузка. После входа проверить сначала
рабочий стол, затем Overview и backdrop blur окон.

## Аварийный откат KWin после чёрного экрана — 2026-08-06, 16:02 MSK

После перезагрузки Plasma Login Manager показывал только чёрный экран.
Причина установлена по журналу текущей загрузки: вручную собранный
`/usr/bin/kwin_wayland` завершался при запуске greeter с ошибкой
`kwin: Неизвестный параметр «no-lockscreen»`. Бинарник для патча Overview
был собран с `-DKWIN_BUILD_SCREENLOCKER=OFF`, однако новый `plasmalogin`
запускает KWin с обязательным аргументом `--no-lockscreen`.

Выполнен безопасный откат через `tools/restore-stock-kwin-overview.sh`:

- восстановлен штатный KWin 6.7.4 из
  `/var/backups/tahoe-kwin/kwin_wayland-6.7.4-stock`;
- SHA-256 `/usr/bin/kwin_wayland` после отката:
  `a0b4937d81e53824746cf4e43812fd3c943baf27fe7e33096224c8c5cd8b7cf8`;
- `plasmalogin.service` перезапущен и активен;
- `plasma-login-kwin_wayland.service`, `plasma-login.service` и
  `plasma-login-wayland.target` успешно запущены;
- greeter подключился к daemon, Wayland socket `wayland-0` создан.

**Текущее состояние:** графический вход восстановлен, используется штатный
KWin. Тема, Kvantum, `forceblur.so` и пользовательские настройки не
откатывались. Патч настоящего backdrop blur в Overview сейчас **не активен**.

**Точка продолжения в графической среде:** войти в Plasma и проверить обычный
рабочий стол/окна. Перед повторной установкой Overview-патча исправить сборку:
не отключать screenlocker (убрать `-DKWIN_BUILD_SCREENLOCKER=OFF`) и отдельно
проверить запуск собранного бинарника с `--no-lockscreen` до замены
`/usr/bin/kwin_wayland`.

## KWin Overview: настоящий backdrop blur — 2026-08-06 (откачен)

Штатный Overview обходит цепочку эффектов при создании `WindowThumbnail`,
поэтому правка `forceblur::shouldBlur()` не могла работать ни до, ни после
перезагрузки. Неудачная правка удалена, стабильный `forceblur` восстановлен.

Был собран и установлен патч самого KWin 6.7.4: под каждым полупрозрачным окном в
Overview рисуется соответствующий участок реально размытого GPU-слоя рабочего
стола (`FastBlur`, radius 64), ограниченный скруглённой маской окна. Штатный
`/usr/bin/kwin_wayland` сохранён для отката. Подробности и контрольные суммы —
`docs/KWIN_OVERVIEW_BLUR.md`. Установка откачена после несовместимости с
`plasmalogin`; визуальная проверка патча не выполнена.
