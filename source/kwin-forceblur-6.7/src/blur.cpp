...
void BlurEffect::reconfigure(ReconfigureFlags flags)
{
    ...
    // Force blur feature: window classes that should be blurred even if they
    // do not request blur themselves (e.g. Alacritty, which cannot get KWin
    // blur via its own protocol request on this KWin version).
    m_windowClasses = BlurConfig::windowClasses();

    for (auto &[window, data] : m_windows) {
        data.blurItem->setPixelsToExpandRepaintsBelowOpaqueRegions(m_expandSize);
    }

    // Re-evaluate every window: force-blur windows may need to be added, and
    // previously force-blurred windows may need to be dropped.
...
void BlurEffect::updateBlurRegion(EffectWindow *w)
{
    ...
    // Force blur feature: if the window matches a configured class and does not
    // already request content blur, blur the whole window (empty region == whole).
    if (!content.has_value() && forceBlurWindow) {
        content = RegionF();
    }

    // Force blur feature (decoration): also blur the frame/titlebar area for
    // matched window classes, since Aurorae never registers its own decoration
    // blur region on this KWin build (see forceDecorationBlurRegion() above).
...
{
    if (m_windowClasses.isEmpty() || w->isDesktop() || w->isDock()) {
        return false;
    }
...
// Radius (device-independent pixels at 1x scale) of the corner rounding
// applied by the separate "Rounded corners" KWin script that MacTahoe-kde's
// README instructs users to enable (window top/bottom corner radius = 24).
// Aurorae itself paints the titlebar background as plain rectangles and
// relies entirely on that script to round the final window shape -- the
// script clips the window's own paint pass, but NOT the separate blur-behind
// quad this effect draws underneath it. Without cutting the same corners out
// of our own region, small square nubs of blurred desktop poke out past the
// rounded titlebar corners (visible live on Dolphin: dark squares outside
// the curve at both top corners). Must be kept in sync if the script's
// configured radius ever changes.
constexpr qreal kForcedCornerRadius = 24.0;

// Subtracts a quarter-circle "outside the round corner" cutout from one
// corner of rect (radius r, clamped to rect's own size). RegionF only stores
// axis-aligned rectangles, so the circle is approximated with one thin
// horizontal strip per row -- the same staircase technique raster region
// implementations use for rounded rects.
static RegionF cutRoundedCorner(const RectF &rect, qreal radius, bool leftEdge, bool topEdge)
{
    const qreal r = std::min({radius, rect.width(), rect.height()});
    if (r <= 0) {
        return RegionF();
    }
    const qreal cornerX = leftEdge ? rect.left() : rect.right();
    const qreal cornerY = topEdge ? rect.top() : rect.bottom();

    RegionF cutout;
    const int rows = std::ceil(r);
    for (int i = 0; i < rows; ++i) {
        const qreal dy = r - (i + 0.5);
        const qreal inside = std::sqrt(std::max<qreal>(0, r * r - dy * dy));
        const qreal w = r - inside;
        if (w <= 0) {
            continue;
        }
        const qreal y = topEdge ? cornerY + i : cornerY - i - 1;
        const qreal x = leftEdge ? cornerX : cornerX - w;
        cutout += RectF(x, y, w, 1);
    }
    return cutout;
}

// Force blur feature: Aurorae (org.kde.kwin.aurorae.v2) never calls
// KDecoration3::Decoration::setBlurRegion(), so decorationSupportsBlurBehind()
// is always false and the whole decoration/titlebar stays sharp even though it
// paints translucent pixels. For force-blurred window classes we compute the
// decoration-minus-contents region ourselves and blur it unconditionally.
RegionF BlurEffect::forceDecorationBlurRegion(const EffectWindow *w) const
{
    if (!w->decoration()) {
        qCWarning(KWIN_BLUR) << "forceDecorationBlurRegion: no decoration for" << w->windowClass();
        return RegionF();
    }
    const RectF decorationRect = w->decoration()->rect();
    RegionF result = RegionF(decorationRect) - w->contentsRect();

    // Only the top corners fall inside this theme's frame region (no side/
    // bottom border), but cutting all four is harmless if a corner has zero
    // overlap with result.
    result -= cutRoundedCorner(decorationRect, kForcedCornerRadius, /*leftEdge=*/true, /*topEdge=*/true);
    result -= cutRoundedCorner(decorationRect, kForcedCornerRadius, /*leftEdge=*/false, /*topEdge=*/true);
    result -= cutRoundedCorner(decorationRect, kForcedCornerRadius, /*leftEdge=*/true, /*topEdge=*/false);
    result -= cutRoundedCorner(decorationRect, kForcedCornerRadius, /*leftEdge=*/false, /*topEdge=*/false);
