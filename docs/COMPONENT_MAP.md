...
## Глобальная прозрачность+блюр (компоузер, не файл темы)

Отдельная категория, не входящая в таблицу выше — она не устанавливается ни
одним из трёх апстримных `install.sh`, а живёт целиком в двух
конфигурационных файлах KWin, которые проект собирает своими скриптами:

| Механизм | Источник в проекте | Устанавливается | Живой путь |
| --- | --- | --- | --- |
| Настоящий blur (не просто альфа) для рамки+содержимого любого окна | `source/kwin-forceblur-6.7/` (патч апстримного KWin blur-эффекта) | `tools/install-kwin-forceblur.sh` | системный `.../qt6/plugins/kwin/effects/plugins/forceblur.so` + `~/.config/kwinrc` |
| Принудительная альфа содержимого окна на уровне компоузера (нужна для окон, которые Kvantum не может сделать прозрачными — System Settings, KCM, GTK/Electron-приложения) | нет отдельного `source/`-файла, генерируется скриптом целиком | `tools/install-kwin-window-rules.sh` | `~/.config/kwinrulesrc` |

Оба — не часть темы MacTahoe как таковой (апстрим ничего подобного не
содержит), это специфичная для этого проекта надстройка. Подробности и
обоснование — `notes/CURRENT_STATE.md`, разделы про System Settings и про
глобальное распространение на все окна; полная воспроизводимая
последовательность установки — `installed/README.md`.

## Активация GTK-темы (отдельно от установки файлов)

`source/MacTahoe-gtk-theme/install.sh` только копирует файлы темы в
`~/.themes/`, не выбирает её активной — выбор темы для GTK-приложений живёт в
четырёх независимых местах (`gtk-3.0/settings.ini`, `gtk-4.0/settings.ini`,
`xsettingsd.conf`, легаси `~/.gtkrc-2.0`), которые ни `install.sh`, ни
`plasma-apply-lookandfeel` не трогают. `tools/apply-gtk-theme.sh`
проставляет тему/иконки/курсор во всех четырёх сразу (и в соответствующих
ключах `gsettings`) — см. `notes/CURRENT_STATE.md`, раздел «GTK-тема сессии
переключена на MacTahoe».
